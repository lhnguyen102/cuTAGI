...
void tagi_idx(IndexOut &idx, Network &net) {
    ...
    std::vector<int> Fmwa_1_pos(num_layers, 0);
    std::vector<int> Fmwa_2_pos(num_layers, 0);
    std::vector<int> FCzwa_1_pos(num_layers, 0);
    std::vector<int> FCzwa_2_pos(num_layers, 0);
    std::vector<int> Szz_ud_pos(num_layers, 0);
    std::vector<int> pooling_pos(num_layers, 0);
    std::vector<int> FCwz_2_pos(num_layers, 0);
    std::vector<int> Swz_ud_pos(num_layers, 0);
    std::vector<int> Fmwa_2_sc_pos(num_layers, 0);
    std::vector<int> FCzwa_1_sc_pos(num_layers, 0);
    std::vector<int> FCzwa_2_sc_pos(num_layers, 0);
    std::vector<int> Szz_ud_sc_pos(num_layers, 0);

    for (int j = 0; j < num_layers - 1; j++) {
        // Convolutional layer
        if (net.layers[j + 1] == net.layer_names.conv &&
            net.similar_layers[j] == j) {
            // Compute padding indices
            int pad_idx_in = net.widths[j] * net.heights[j] * net.filters[j] *
                                 net.batch_size +
                             1;
            int pad_idx_out = net.widths[j + 1] * net.heights[j + 1] *
                              ...
                              1;
            int param_pad_idx =
                pow(net.kernels[j], 2) * net.filters[j] * net.filters[j + 1] +
                1;

            // Get convolutional indices
            auto convIndex = get_conv_idx(
                net.kernels[j], net.strides[j], net.widths[j], net.heights[j],
                net.widths[j + 1], net.heights[j + 1], net.pads[j],
                net.pad_types[j], pad_idx_in, pad_idx_out, param_pad_idx);

            // Store indices to the main vector for all layers
            idx.Fmwa_2.insert(idx.Fmwa_2.end(), convIndex.Fmwa_2_idx.begin(),
                              convIndex.Fmwa_2_idx.end());

            idx.FCzwa_1.insert(idx.FCzwa_1.end(), convIndex.FCzwa_1_idx.begin(),
                               convIndex.FCzwa_1_idx.end());

            idx.Szz_ud.insert(idx.Szz_ud.end(), convIndex.Szz_ud_idx.begin(),
                              convIndex.Szz_ud_idx.end());

            // Save postion of the index vector in the main vector
            Fmwa_2_pos[j + 1] = convIndex.Fmwa_2_idx.size();
            FCzwa_1_pos[j + 1] = convIndex.FCzwa_1_idx.size();
            Szz_ud_pos[j + 1] = convIndex.Szz_ud_idx.size();
            net.row_zw[j] = convIndex.h;
            net.col_z_ud[j] = convIndex.h;
        }

        // Transpose convolutional layer
        else if (net.layers[j + 1] == net.layer_names.tconv) {
            // Compute padding indices
            int pad_idx_in = net.widths[j + 1] * net.heights[j + 1] *
                                 net.filters[j + 1] * net.batch_size +
                             1;
            int pad_idx_out = net.widths[j] * net.heights[j] * net.filters[j] *
                                  net.batch_size +
                              1;
            int param_pad_idx =
                pow(net.kernels[j], 2) * net.filters[j] * net.filters[j + 1] +
            ...
            // Get tranpose convolutional indices
            auto convIndex = get_conv_idx(
                net.kernels[j], net.strides[j], net.widths[j + 1],
                net.heights[j + 1], net.widths[j], net.heights[j], net.pads[j],
                net.pad_types[j], pad_idx_in, pad_idx_out, param_pad_idx);

            auto tconvIndex = get_tconv_idx(
                net.kernels[j], net.widths[j + 1], net.heights[j + 1],
                net.widths[j + 1], net.heights[j + 1], pad_idx_in, pad_idx_out,
                param_pad_idx, convIndex);

            // Store in main vector for all layers
            std::vector<int> tmp_tconv = transpose_matrix(
                convIndex.Szz_ud_idx, convIndex.w, convIndex.h);

            idx.Fmwa_1.insert(idx.Fmwa_1.end(), convIndex.FCzwa_1_idx.begin(),
                              convIndex.FCzwa_1_idx.end());

            idx.Fmwa_2.insert(idx.Fmwa_2.end(), tmp_tconv.begin(),
                              tmp_tconv.end());

            idx.FCwz_2.insert(idx.FCwz_2.end(), tconvIndex.FCwz_2_idx.begin(),
                              tconvIndex.FCwz_2_idx.end());

            idx.Swz_ud.insert(idx.Swz_ud.end(), tconvIndex.Swz_ud_idx.begin(),
                              tconvIndex.Swz_ud_idx.end());

            idx.FCzwa_1.insert(idx.FCzwa_1.end(),
                               tconvIndex.FCzwa_1_idx.begin(),
                               tconvIndex.FCzwa_1_idx.end());

            idx.Szz_ud.insert(idx.Szz_ud.end(), tconvIndex.Szz_ud_idx.begin(),
                              tconvIndex.Szz_ud_idx.end());

            // Save position of the index vector in the main vector
            Fmwa_1_pos[j + 1] = convIndex.FCzwa_1_idx.size();
            Fmwa_2_pos[j + 1] = convIndex.Szz_ud_idx.size();
            FCwz_2_pos[j + 1] = tconvIndex.FCwz_2_idx.size();
            Swz_ud_pos[j + 1] = tconvIndex.Swz_ud_idx.size();
            FCzwa_1_pos[j + 1] = tconvIndex.FCzwa_1_idx.size();
            Szz_ud_pos[j + 1] = tconvIndex.Szz_ud_idx.size();
            net.row_zw[j] = tconvIndex.w_wz;
            net.col_z_ud[j] = tconvIndex.w_zz;
            net.Fmwa_1_col[j] = convIndex.h;
            net.FCzwa_1_col[j] = tconvIndex.w_wz;

        }

        // Average Pooling layers
        else if (net.layers[j + 1] == net.layer_names.ap) {
            // Compute padding indices
            int pad_idx_in = net.widths[j] * net.heights[j] * net.filters[j] *
                                 net.batch_size +
                             1;
            ...
            // Get pooling indices
            auto poolIndex = get_pool_idx(
                net.kernels[j], net.strides[j], net.widths[j], net.heights[j],
                net.widths[j + 1], net.heights[j + 1], net.pads[j],
                net.pad_types[j], pad_idx_in, pad_idx_out);

            // Store in the main vector all layers
            idx.pooling.insert(idx.pooling.end(), poolIndex.pool_idx.begin(),
                               poolIndex.pool_idx.end());

            idx.Szz_ud.insert(idx.Szz_ud.end(), poolIndex.Szz_ud_idx.begin(),
                              poolIndex.Szz_ud_idx.end());

            // Save position of the index vector in the main vector
            pooling_pos[j + 1] = poolIndex.pool_idx.size();
            Szz_ud_pos[j + 1] = poolIndex.Szz_ud_idx.size();
            net.row_zw[j] = poolIndex.w;
            net.col_z_ud[j] = poolIndex.h;
        }
        ...
        if (net.shortcuts[j] > -1 &&
            ...
            // Compute padding indices
            int idx_sc = net.shortcuts[j];
            int pad_idx_in = net.widths[idx_sc] * net.heights[idx_sc] *
                                 net.filters[idx_sc] * net.batch_size +
                             1;
            int pad_idx_out = net.widths[j + 1] * net.heights[j + 1] *
                                  net.filters[j + 1] * net.batch_size +
                              1;
            int param_pad_idx = net.filters[idx_sc] * net.filters[j + 1] + 1;

            // Get convolutional indices
            auto convIndex = get_conv_idx(
                1, net.strides[idx_sc], net.widths[idx_sc], net.heights[idx_sc],
                net.widths[j + 1], net.heights[j + 1], 0, 1, pad_idx_in,
                pad_idx_out, param_pad_idx);

            // Store indices to the main vector for all layers
            std::vector<int> tmp_sc = transpose_matrix(
                convIndex.FCzwa_2_idx, convIndex.w, convIndex.h);

            idx.Fmwa_2_sc.insert(idx.Fmwa_2_sc.end(),
                                 convIndex.Fmwa_2_idx.begin(),
                                 convIndex.Fmwa_2_idx.end());
            idx.FCzwa_1_sc.insert(idx.FCzwa_1_sc.end(),
                                  convIndex.FCzwa_1_idx.begin(),
                                  convIndex.FCzwa_1_idx.end());

            idx.FCzwa_2_sc.insert(idx.FCzwa_2_sc.end(), tmp_sc.begin(),
                                  tmp_sc.begin() + convIndex.w);

            idx.Szz_ud_sc.insert(idx.Szz_ud_sc.end(),
                                 convIndex.Szz_ud_idx.begin(),
                                 convIndex.Szz_ud_idx.end());

            // Save postion of the index vector in the main vector
            Fmwa_2_sc_pos[net.shortcuts[j] + 1] = convIndex.Fmwa_2_idx.size();
            FCzwa_1_sc_pos[net.shortcuts[j] + 1] = convIndex.FCzwa_1_idx.size();
            FCzwa_2_sc_pos[net.shortcuts[j] + 1] = convIndex.FCzwa_1_idx.size();
            Szz_ud_sc_pos[net.shortcuts[j] + 1] = convIndex.Szz_ud_idx.size();
            net.row_w_sc[net.shortcuts[j]] = convIndex.w;
            net.col_z_sc[net.shortcuts[j]] = convIndex.h;
        }

        // NOTE: current version does not support max pooling layer. Future
        // version will address this issue
    }
    // Cumsum the index position vector
    net.Fmwa_1_pos = cumsum(Fmwa_1_pos);
    net.Fmwa_2_pos = cumsum(Fmwa_2_pos);
    net.FCzwa_1_pos = cumsum(FCzwa_1_pos);
    net.FCzwa_2_pos = cumsum(FCzwa_2_pos);
    net.Szz_ud_pos = cumsum(Szz_ud_pos);
    net.pooling_pos = cumsum(pooling_pos);
    net.FCwz_2_pos = cumsum(FCwz_2_pos);
    net.Swz_ud_pos = cumsum(Swz_ud_pos);
    net.Fmwa_2_sc_pos = cumsum(Fmwa_2_sc_pos);
    net.FCzwa_1_sc_pos = cumsum(FCzwa_1_sc_pos);
    net.FCzwa_2_sc_pos = cumsum(FCzwa_2_sc_pos);
    net.Szz_ud_sc_pos = cumsum(Szz_ud_sc_pos);

    // Similar layers
    set_idx_to_similar_layer(net.similar_layers, net.Fmwa_2_pos);
    set_idx_to_similar_layer(net.similar_layers, net.FCzwa_1_pos);
    set_idx_to_similar_layer(net.similar_layers, net.Szz_ud_pos);
    set_idx_to_similar_layer(net.similar_layers, net.row_zw);
    set_idx_to_similar_layer(net.similar_layers, net.col_z_ud);
}
