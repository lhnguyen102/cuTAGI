...
void forget_gate_cpu(Network &net, NetState &state, Param &theta, int l) {
    ...
    if (net.multithreading && no_b_seq > net.min_operations) {
        fc_mean_var_multithreading(
            ...
            state.lstm.Sf_ga);
        mixture_sigmoid_multithreading(state.lstm.mf_ga, state.lstm.Sf_ga,
                                       net.z_pos_lstm[l], no_b_seq,
        ...
        mixture_sigmoid_cpu(state.lstm.mf_ga, state.lstm.Sf_ga,
...
void input_gate_cpu(Network &net, NetState &state, Param &theta, int l) {
    ...
    if (net.multithreading && no_b_seq > net.min_operations) {
        fc_mean_var_multithreading(
            ...
            state.lstm.Si_ga);
        mixture_sigmoid_multithreading(state.lstm.mi_ga, state.lstm.Si_ga,
                                       net.z_pos_lstm[l], no_b_seq,
        ...
        mixture_sigmoid_cpu(state.lstm.mi_ga, state.lstm.Si_ga,
...
void cell_state_gate_cpu(Network &net, NetState &state, Param &theta, int l) {
    ...
    if (net.multithreading && no_b_seq > net.min_operations) {
        ...
        mixture_tanh_cpu(state.lstm.mc_ga, state.lstm.Sc_ga, net.z_pos_lstm[l],
                         0, no_b_seq, state.lstm.mc_ga, state.lstm.Jc_ga,
                         state.lstm.Sc_ga);
    }
}
...
void output_gate_cpu(Network &net, NetState &state, Param &theta, int l) {
    ...
    if (net.multithreading && no_b_seq > net.min_operations) {
        fc_mean_var_multithreading(
            ...
            state.lstm.So_ga);
        mixture_sigmoid_multithreading(state.lstm.mo_ga, state.lstm.So_ga,
                                       net.z_pos_lstm[l], no_b_seq,
        ...
        mixture_sigmoid_cpu(state.lstm.mo_ga, state.lstm.So_ga,
...
// TODO: Fix cycling import between feed_forward_cpu and lstm_feed_forward_cpu
{
    ...
    if (net.multithreading && no_b_seq > net.min_operations) {
        ...
        mixture_tanh_multithreading(state.lstm.mc, state.lstm.Sc, z_pos_o_lstm,
        ...
        cell_state_mean_var_cpu(
            ...
            state.lstm.Sc);
        mixture_tanh_cpu(state.lstm.mc, state.lstm.Sc, z_pos_o_lstm, 0,
                         no_b_seq, state.lstm.mca, state.lstm.Jca,
                         state.lstm.Sca);
