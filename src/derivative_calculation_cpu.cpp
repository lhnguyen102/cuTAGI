...
void compute_cov_dd_dw_fc(std::vector<float> &ma, std::vector<float> &Sa,
                          std::vector<float> &J, std::vector<float> &mw,
                          std::vector<float> &Sw, int act_i, int act_o,
                          int w_pos_i, int z_pos_i, int z_pos_o, int ni, int no,
                          int B, std::vector<float> &Cdo_di,
    ...
    if (act_o == 1)  // Tanh
    {
        for (int j = 0; j < no; j++) {
            for (int i = 0; i < ni * B; i++) {
                m = (i / ni) * no + j;
                k = (i / ni) * j * B;
                Cdo_wi[ni * B * j + i] = -2.0f * ma[m + z_pos_o] *
                                         mw[k + w_pos_i] * Sa[i + z_pos_i] *
                                         J[m + z_pos_o];
            }
        }
...
void compute_cov_d_dw_dw_fc(std::vector<float> &md_n, std::vector<float> &mw_o,
                            std::vector<float> &Cdo_diwi, int w_pos_o, int ni,
                            int no, int nn, int B,
                            std::vector<float> &Cdn_dowo_diwi)
...
void compute_layer_derivative_mean_var_fc(
    std::vector<float> &md_node, std::vector<float> &Sd_node,
    std::vector<float> &md_layer_o, std::vector<float> &Sd_layer_o,
    std::vector<float> &md_layer_m_o, std::vector<float> &md_layer_n,
    std::vector<float> &mw_o, std::vector<float> &Cdo_diwi, int z_pos_o,
    int z_pos_n, int ni, int no, int nn, int B, std::vector<float> &md_layer_m,
    ...
    for (int j = 0; j < no; j++) {
        for (int i = 0; i < ni * B; i++) {
            ...
            for (int k = 0; k < nn; k++) {
                ...
                tmp_cov = md_layer_n[k + (i / ni) * nn + z_pos_n] *
                          mw_o[k * no] * Cdo_diwi[i + j * ni * B];
            ...
            m = (i / ni) * no + j;
            md_layer_m[ni * B * j + i] =
                sum_mean + md_node[ni * B * j + i] * md_layer_o[m + z_pos_o];
            Sd_layer_m[ni * B * j + i] =
                sum_cov + Sd_node[ni * B * j + i] * Sd_layer_o[m + z_pos_o] +
                Sd_layer_o[m + z_pos_o] * md_node[ni * B * j + i] *
                    md_node[ni * B * j + i];
        }
    }
}
...
void compute_cov_dz(std::vector<float> &ma, std::vector<float> &Cai_zi,
                    std::vector<float> &Cao_zi, int act_o, int act_i,
                    int z_pos_i, int z_pos_o, int ni, int no, int B,
   ...
   hidden states of the current layer*/
{
    ...
    if (act_i == 1)  // Tanh
    {
        for (int j = 0; j < no; j++) {
            for (int i = 0; i < ni * B; i++) {
                Cdi_zi[ni * B * j + i] =
                    -2.0f * ma[i + z_pos_i] * Cai_zi[ni * B * j + i];
            }
        }
    } else if (act_i == 2)  // sigmoid
    {
        for (int j = 0; j < no; j++) {
            for (int i = 0; i < ni * B; i++) {
                Cdi_zi[ni * B * j + i] =
                    (1.0f - 2.0f * ma[i + z_pos_i]) * Cai_zi[ni * B * j + i];
            }
        }
    } else {
        for (int i = 0; i < no * ni * B; i++) {
    ...
    if (act_o == 1)  // Tanh
    {
        for (int j = 0; j < no; j++) {
            for (int i = 0; i < ni * B; i++) {
                m = (i / ni) * no + j;
                Cdo_zi[ni * B * j + i] =
                    -2.0f * ma[m + z_pos_o] * Cao_zi[ni * B * j + i];
            }
        }
    } else if (act_o == 2)  // Sigmoid
    {
        for (int j = 0; j < no; j++) {
            for (int i = 0; i < ni * B; i++) {
                m = (i / ni) * no + j;
                Cdo_zi[ni * B * j + i] =
                    (1.0f - 2.0f * ma[m + z_pos_o]) * Cao_zi[ni * B * j + i];
            }
        }
    } else {
        for (int i = 0; i < no * ni * B; i++) {
...
void compute_last_layer_cov(std::vector<float> &mw,
                            std::vector<float> &md_layer_n,
                            std::vector<float> &md_node,
                            std::vector<float> &md_layer_m_o,
                            std::vector<float> &Cdi_zi,
                            std::vector<float> &Cdo_zi, int w_pos_i,
                            int w_pos_o, int z_pos_n, int ni, int no, int nn,
   ...
   the current layers*/
{
    ...
    // Compute covariance between next activations & weights and  current
    // activations
    compute_cov_dd_dw_fc(state.ma, state.Sa, state.J, theta.mw, theta.Sw, act_i,
                         act_o, w_pos_i, z_pos_i, z_pos_o, ni, no,
                         net.batch_size, state.derv_state.Cdo_di,
                         state.derv_state.Cdo_wi);
...
void tanh_dervatives(std::vector<float> &ma, std::vector<float> &Sa,
...
void relu_derivative(std::vector<float> &mz, int z_pos, int n,
                     std::vector<float> &mda, std::vector<float> &Sda) {
    for (int i = 0; i < n; i++) {
        if (mz[i + z_pos] > 0) {
            mda[i + z_pos] = 1.0f;
...
// MAIN
////////////////////////////////////////////////////////////////////////////////
void get_derivative(Network &net, Param &theta, NetState &state, int layer) {}