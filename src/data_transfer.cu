...
void StateGPU::set_values(NetState &state) {
...
void ParamGPU::allocate_cuda_memory() {
    ...
    cudaError_t error = cudaGetLastError();
    if (error != cudaSuccess) {
        std::string err_msg = "Failed to allocate CUDA memory for parameters";
        std::cerr << error << ": " << err_msg;
    }
}
...
////////////////////////////////
// ObsGPU::ObsGPU(int nx, int ny, int nye, int B)
//{
//   id_bytes = B * nx * sizeof(float);
...
//   if (error != cudaSuccess)
//   {
//     std::string err_msg = "Failed to allocate CUDA memory for inputs and
//     outputs"; std::cerr << error << ": " << err_msg;
//   }
// }
//
// void ObsGPU::copy_host_to_device(std::vector<float> &x_batch,
...
//   if (error != cudaSuccess)
//   {
//     std::string err_msg = "Failed to make data transfer to device for inputs
//     and outputs"; std::cerr << error << ": " << err_msg;
//   }
// }
//
// void ObsGPU::copy_device_to_host(std::vector<float> &x_batch,
//     std::vector<float> Sx_batch, std::vector<float> &y_batch,
//     std::vector<int> &idx_ud_batch, std::vector<float> &V_batch)
//{
...
//   if (error != cudaSuccess)
//   {
//     std::string err_msg = "Failed to make data transfer to host for inputs
//     and outputs"; std::cerr << error << ": " << err_msg;
//   }
// }
//
...
// INPUT
//////////////////////////////
InputGPU::InputGPU(int nx, int B) {
    id_bytes = B * nx * sizeof(float);
...
void InputGPU::allocate_cuda_memory() {
    ...
    cudaError_t error = cudaGetLastError();
    if (error != cudaSuccess) {
        std::string err_msg = "Failed to allocate CUDA memory for inputs";
        std::cerr << error << ": " << err_msg;
    }
}
...
void ObsGPU::allocate_cuda_memory() {
    ...
    cudaError_t error = cudaGetLastError();
    if (error != cudaSuccess) {
        std::string err_msg = "Failed to allocate CUDA memory for outputs";
        std::cerr << error << ": " << err_msg;
    }
}
