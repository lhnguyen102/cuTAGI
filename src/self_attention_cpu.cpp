...
//                             std::vector<float> &var_in, int in_pos, int
//                             out_pos, int batch_size, int num_heads, int
//                             timestep, int head_size, std::vector<float>
//                             &mu_out, std::vector<float> &var_out)
...
//                         int a_pos, int b_pos, int ab_pos, int N, int C, int
//                         H, int W, int D, std::vector<float> &mu_ab,
...
//                         a_pos; idx_b = i * C * H * W + j * H * W + l + m * W
//                         + b_pos;
...
//                std::vector<float> &mu_k, std::vector<float> &var_k, int
//                qkv_pos, int batch_size, int num_heads, int timestep, int
//                head_size, std::vector<float> &mu_qk, std::vector<float>
//                &var_qk)
...
//                     int batch_size, int num_heads, int timestep, int
//                     head_size, std::vector<float> &mu_mqk, std::vector<float>
//                     &var_mqk) {
...
//                         if (m <= k) {
//                             idx_qk = i * num_heads * timestep * timestep +
//                                      j * timestep * timestep + m * timestep +
//                                      l;
//                             sum_mu += mu_qk[idx_qk];
//                             sum_var +=
...
// /*The ordering of the embedding vectors are query, key, and values with a
// space
//    of comp_size */
// {
...
//                  z_sum_remax_pos, timestep, timestep * num_heads *
//                  batch_size);
...
//                state.mha.var_out_proj, w_out_proj_pos, b_out_proj_pos,
//                qkv_pos, z_pos_out, num_embs, num_embs, batch_size * timestep,
//                state.Sz);
//     int check = 1;
// }
...
//                      int batch_size, int num_heads, int timestep, int
//                      head_size, std::vector<float> &delta_mu_s,
...
//                                   j * timestep * head_size + k * head_size +
//                                   m;
...
//                      std::vector<float> &delta_var, int qkv_pos, int att_pos,
//                      int batch_size, int num_heads, int timestep, int
//                      head_size, std::vector<float> &delta_mu_v,
...
//                      std::vector<float> &delta_var, int qkv_pos, int att_pos,
//                      int batch_size, int num_heads, int timestep, int
//                      head_size, std::vector<float> &delta_mu_q,
...
//  * for the query's hidden states, based on the given input variables. It
//  takes
...
//                                     + m + qkv_pos;
//                             idx_s = i * num_heads * timestep * timestep +
//                                     j * timestep * timestep + k * timestep +
//                                     l + att_pos;
...
//                    std::vector<float> &delta_mu, std::vector<float>
//                    &delta_var, int qkv_pos, int att_pos, int batch_size, int
//                    num_heads, int timestep, int head_size, std::vector<float>
//                    &delta_mu_k, std::vector<float> &delta_var_k)
// /**
...
//                     delta_mu_k[idx_q] =
//                         sum_mu * mu_q[idx_q + qkv_pos] / powf(num_heads,
//                         0.5);
...
//                 d_state.delta_m, w_out_proj_pos, qkv_pos, z_pos_out,
//                 num_embs, num_embs, batch_timestep,
//                 d_state.mha.delta_mu_buffer);
//     fc_delta_Sz(theta.mw, state.mha.var_out_proj, state.mha.J_out_proj,
//                 d_state.delta_S, w_out_proj_pos, qkv_pos, z_pos_out,
//                 num_embs, num_embs, batch_timestep,
...
//                     d_state.mha.delta_mu_buffer,
//                     d_state.mha.delta_var_buffer, qkv_pos, att_pos,
//                     batch_size, num_heads, timestep, head_size,
//                     d_state.mha.delta_mu_v, d_state.mha.delta_var_v);
...
//         d_state.mha.delta_mu_v, d_state.mha.delta_var_v, 0, in_proj_pos,
//         batch_size, num_heads, timestep, head_size,
//         d_state.mha.delta_mu_in_proj, d_state.mha.delta_var_in_proj);
...
//     fc_delta_mb(theta.Sb, d_state.delta_m, b_out_proj_pos, z_pos_out,
//     num_embs,
//                 batch_timestep, 1, d_theta.delta_mb);
//     fc_delta_mb(theta.Sb, d_state.delta_S, b_out_proj_pos, z_pos_out,
//     num_embs,
//                 batch_timestep, 1, d_theta.delta_Sb);

//     // Parameters for input projections. TODO: need to revise the postion for
//     // delta_mu_in_proj
//     fc_delta_mw(theta.Sw, state.ma, d_state.mha.delta_mu_in_proj,
//     w_in_proj_pos,
//                 z_pos_in, in_proj_pos, num_embs, batch_timestep, num_embs,
//                 d_theta.delta_mw);
//     fc_delta_Sw(theta.Sw, state.ma, d_state.mha.delta_var_in_proj,
//                 w_in_proj_pos, z_pos_in, in_proj_pos, num_embs,
//                 batch_timestep, num_embs, d_theta.delta_Sw);
