...
#               "Kin8nm","Naval",\
#               "Power-plant","Protein"]
data_names = ["Naval", "Power-plant","Protein"] # "Boston_housing","Concrete", "Energy", "Yacht", "Wine", "Kin8nm","Naval", "Power-plant","Protein"

for j in range(len(data_names)):

    # check if the results folder already exists; create it if does not exist or remove the existing one
    if not os.path.exists("results_small_UCI_TAGI_earlystop/{}".format(data_names[j])):
        os.makedirs("results_small_UCI_TAGI_earlystop/{}".format(data_names[j]))
    elif os.path.isfile("results_small_UCI_TAGI_earlystop/{}/RMSEtest.txt".format(data_names[j])) and \
        os.path.isfile("results_small_UCI_TAGI_earlystop/{}/LLtest.txt".format(data_names[j])) and \
        os.path.isfile("results_small_UCI_TAGI_earlystop/{}/optimal_epoch.txt".format(data_names[j])):

        os.remove("results_small_UCI_TAGI_earlystop/{}/RMSEtest.txt".format(data_names[j]))
        os.remove("results_small_UCI_TAGI_earlystop/{}/LLtest.txt".format(data_names[j]))
        os.remove("results_small_UCI_TAGI_earlystop/{}/runtime_train.txt".format(data_names[j]))
        os.remove("results_small_UCI_TAGI_earlystop/{}/optimal_epoch.txt".format(data_names[j]))
    ...
    RESULTS_RMSEtest = "results_small_UCI_TAGI_earlystop/"+data_names[j]+"/RMSEtest.txt"
    RESULTS_LLtest = "results_small_UCI_TAGI_earlystop/"+data_names[j]+"/LLtest.txt"
    RESULTS_RUNTIME = "results_small_UCI_TAGI_earlystop/"+data_names[j]+"/runtime_train.txt"
    RESULTS_EPOCH = "results_small_UCI_TAGI_earlystop/"+data_names[j]+"/optimal_epoch.txt"
