...
## Running for each dataset in data_names
for j in range(len(data_names)):
    ...
    elif os.path.isfile("results_small_UCI_TAGI/{}/RMSEtest_opt_Epoch.txt".format(data_names[j])) and \
        os.path.isfile("results_small_UCI_TAGI/{}/LLtest_opt_Epoch.txt".format(data_names[j])) and \
        os.path.isfile("results_small_UCI_TAGI/{}/runtime_train_opt_Epoch.txt".format(data_names[j])):

        os.remove("results_small_UCI_TAGI/{}/RMSEtest_opt_Epoch.txt".format(data_names[j]))
        os.remove("results_small_UCI_TAGI/{}/LLtest_opt_Epoch.txt".format(data_names[j]))
        os.remove("results_small_UCI_TAGI/{}/runtime_train_opt_Epoch.txt".format(data_names[j]))


    # File paths for the results
    RESULTS_RMSEtest = "results_small_UCI_TAGI/"+data_names[j]+"/RMSEtest_opt_Epoch.txt"
    RESULTS_LLtest = "results_small_UCI_TAGI/"+data_names[j]+"/LLtest_opt_Epoch.txt"
    RESULTS_RUNTIME = "results_small_UCI_TAGI/"+data_names[j]+"/runtime_train_opt_Epoch.txt"
    # getting data name
    ...
    # results are from the folder "results_small_UCI_TAGI_earlystop"
    # EPOCHS = {"Boston_housing": 13, "Concrete": 15, "Energy": 36, "Yacht": 25, "Wine": 7, \
    #                     "Kin8nm": 25, "Naval": 24, "Power-plant": 9, "Protein": 10}
    ...
    for loop in range(total_run):
        ...
        for i in range(n_splits):
            ...
            # converting the train and test values to standardized space
