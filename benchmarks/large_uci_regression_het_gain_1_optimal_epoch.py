###############################################################################
# File:         large_uci_regression_het_gain_1_optimal_epoch.py
# Description:  Running large uci datasets until the optimal epoch
...
total_run = 3 # define the number of runs

for j in range(len(data_names)):

    # check if the results folder already exists; create it if does not exist or remove the existing one
    if not os.path.exists("benchmarks/logs/results_large_uci_regression_het_gain_1_optimal_epoch/{}".format(data_names[j])):
        os.makedirs("benchmarks/logs/results_large_uci_regression_het_gain_1_optimal_epoch/{}".format(data_names[j]))
    elif os.path.isfile("benchmarks/logs/results_large_uci_regression_het_gain_1_optimal_epoch/{}/RMSEtest_opt_Epoch.txt".format(data_names[j])) and \
        os.path.isfile("benchmarks/logs/results_large_uci_regression_het_gain_1_optimal_epoch/{}/LLtest_opt_Epoch.txt".format(data_names[j])) and \
        os.path.isfile("benchmarks/logs/results_large_uci_regression_het_gain_1_optimal_epoch/{}/runtime_train_opt_Epoch.txt".format(data_names[j])):

        os.remove("benchmarks/logs/results_large_uci_regression_het_gain_1_optimal_epoch/{}/RMSEtest_opt_Epoch.txt".format(data_names[j]))
        os.remove("benchmarks/logs/results_large_uci_regression_het_gain_1_optimal_epoch/{}/LLtest_opt_Epoch.txt".format(data_names[j]))
        os.remove("benchmarks/logs/results_large_uci_regression_het_gain_1_optimal_epoch/{}/runtime_train_opt_Epoch.txt".format(data_names[j]))


    # File paths for the results
    RESULTS_RMSEtest = "benchmarks/logs/results_large_uci_regression_het_gain_1_optimal_epoch/"+data_names[j]+"/RMSEtest_opt_Epoch.txt"
    RESULTS_LLtest = "benchmarks/logs/results_large_uci_regression_het_gain_1_optimal_epoch/"+data_names[j]+"/LLtest_opt_Epoch.txt"
    RESULTS_RUNTIME = "benchmarks/logs/results_large_uci_regression_het_gain_1_optimal_epoch/"+data_names[j]+"/runtime_train_opt_Epoch.txt"
    ...
    filepath_opt_epoch = "benchmarks/logs/results_large_uci_regression_het_gain_1_earlystop/"+data_names[j]+"/optimal_epoch.txt"
    ...
    # optimal epochs for each dataset
    ...
    class HeterosUCIMLP(NetProp):
        ...
        def __init__(self) -> None:
            ...
            self.sigma_v_min    =  0
            self.out_gain       =  1.0 #OUT_GAIN[data_names[j]]
            self.noise_gain     =  1.0 #NOISE_GAIN[data_names[j]]
    ...
    mse_runs, rmse_runs, normal_log_lik_runs, runtime_runs = [],[],[],[]
    for loop in range(total_run):
        ...
        for i in range(n_splits):
            ...
            _, rmse_Epochlist, _, normal_LL_Epochlist, _ = reg_task.train_UCI()
            ...
            mse, _, rmse, normal_LL = reg_task.predict_UCI()
            # Store the results
            mse_list.append(mse)
        ...
        mean_LL   = np.mean(normal_LL_splitlist,axis=0)


        # Print the average results
        print("Average MSE: {:.3f} ± {:.3f}".format(np.mean(mse_list), np.std(mse_list)))
        print("Average normal Log-likelihood: {:.3f} ± {:.3f}".format(np.mean(normal_log_lik_list), np.std(normal_log_lik_list)))
        ...
        normal_log_lik_runs.append((np.mean(normal_log_lik_list), np.std(normal_log_lik_list)))
        runtime_runs.append((np.mean(runtime_list), np.std(runtime_list)))



    # Check for NaN values in rmse_runs and log_lik_runs
    if np.isnan(rmse_runs).any() or np.isnan(normal_log_lik_runs).any():
    ...
    with open(RESULTS_LLtest, "a") as file:
        for mean, std in normal_log_lik_runs:
            file.write("{:.3f} ± {:.3f}\n".format(mean, std))
