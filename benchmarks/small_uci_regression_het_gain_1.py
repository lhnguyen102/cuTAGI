###############################################################################
# File:         small_uci_regression_het_gain_1.py
# Description:  Running small uci datasets with heteroscedasticity for user-defined epochs with both gains equal to 1
...
for j in range(len(data_names)):
    ...
    if not os.path.exists("benchmarks/logs/results_small_uci_regression_het_gain_1/{}".format(data_names[j])):
        os.makedirs("benchmarks/logs/results_small_uci_regression_het_gain_1/{}".format(data_names[j]))
    elif os.path.isfile("benchmarks/logs/results_small_uci_regression_het_gain_1/{}/RMSEtest.txt".format(data_names[j])) and \
        os.path.isfile("benchmarks/logs/results_small_uci_regression_het_gain_1/{}/LLtest.txt".format(data_names[j])) and \
        os.path.isfile("benchmarks/logs/results_small_uci_regression_het_gain_1/{}/runtime_train.txt".format(data_names[j])):

        os.remove("benchmarks/logs/results_small_uci_regression_het_gain_1/{}/RMSEtest.txt".format(data_names[j]))
        os.remove("benchmarks/logs/results_small_uci_regression_het_gain_1/{}/LLtest.txt".format(data_names[j]))
        os.remove("benchmarks/logs/results_small_uci_regression_het_gain_1/{}/runtime_train.txt".format(data_names[j]))


    # File paths for the results
    RESULTS_RMSEtest = "benchmarks/logs/results_small_uci_regression_het_gain_1/"+data_names[j]+"/RMSEtest.txt"
    RESULTS_LLtest = "benchmarks/logs/results_small_uci_regression_het_gain_1/"+data_names[j]+"/LLtest.txt"
    RESULTS_RUNTIME = "benchmarks/logs/results_small_uci_regression_het_gain_1/"+data_names[j]+"/runtime_train.txt"
    ...
    filepath_opt_epoch = "results_small_uci_regression_het_gain_1_earlystop/"+data_names[j]+"/optimal_epoch.txt"
    ...
    optimal_epoch_path = "benchmarks/logs/results_small_uci_regression_het_gain_1_earlystop/"+ data_names[j] + "/optimal_epoch.txt"
    if os.path.exists(optimal_epoch_path):
        ...
        if os.path.exists(optimal_epoch_path):
            ax[0].axvline(x=optimal_epoch, color='r', linestyle='--')  # Dashed line for optimal epoch on RMSE plot
        ax[0].set_xlabel('Epochs')
        ax[0].set_ylabel('RMSE')

        ax[1].scatter(range(num_epochs), mean_LL)
        if os.path.exists(optimal_epoch_path):
            ax[1].axvline(x=optimal_epoch, color='r', linestyle='--')  # Dashed line for optimal epoch on Log-likelihood plot
        ...
        plt.savefig("benchmarks/logs/results_small_uci_regression_het_gain_1/" + data_names[j] + "/RMSE_LL.png")
