...
datasets = ["elevators","skillcraft","pol", "keggdirected", "keggundirected"]

data = []

for dataset in datasets:

    folder_path = os.path.join("benchmarks/logs/results_large_uci_regression_het_gain_1_optimal_epoch", dataset)
...
# Creating DataFrame
df = pd.DataFrame(data, columns=["Dataset", "RMSE", "Log-Likelihood", "Average Training Time per split (in s.)"])

# upto 3 decimal places
df_rounded = df.round(3)

# saving table as a markdown file (.txt)
with open("benchmarks/logs/results_large_uci_regression_het_gain_1_optimal_epoch/large_uci_regression_het_gain_1_table.txt", "w") as f:
