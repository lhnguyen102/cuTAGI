...
def build_mingpt(
    ...
    layers.append(Linear(embed_dim, vocab_size, bias=False))
    layers.append(Remax())
...
def main(
    ...
    using Remax for the output distribution."""
    ...
    for epoch in pbar:
        ...
        for _ in range(steps_per_epoch):
            ...
            prob = np.asarray(m_pred).reshape(num_samples, vocab_size)
            predicted = np.argmax(prob, axis=1)
            error_rates.append(float(np.mean(predicted != labels)))
    ...
    for _ in range(max_new_tokens):
        ...
        m_pred, _ = net(x)
        probs = np.asarray(m_pred).reshape(seq_len, vocab_size)[-1]
