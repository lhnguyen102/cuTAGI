...
def sampled_metrics(utils, hrc, m_pred, v_pred, labels, obs_scale, num_samples):
    ...
    full 50k-class probability for every position is too expensive. Also
    returns the mean raw probability mass (sum over classes before the
    renormalization) -- 1.0 means a proper distribution."""
    n = len(labels)
