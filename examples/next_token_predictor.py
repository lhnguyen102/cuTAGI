...
def build_mingpt(
    ...
    for _ in range(num_layers):
        ...
        ffn_lin1 = Linear(embed_dim, ffn_hidden, bias=False)
        ffn_lin2 = Linear(ffn_hidden, embed_dim, bias=False)
        ffn_lin1._cpp_backend.debug = debug
        ffn_lin1._cpp_backend.debug_interval = debug_interval
        ffn_lin2._cpp_backend.debug = debug
        ffn_lin2._cpp_backend.debug_interval = debug_interval
        layers.append(
            ResNetBlock(
                LayerBlock(
                    RMSNorm(
    ...
    head = Linear(embed_dim, output_size)
    head._cpp_backend.debug = debug
    head._cpp_backend.debug_interval = debug_interval
    layers.append(head)
...
def main(
    ...
    for epoch in pbar:
        ...
        for _ in range(steps_per_epoch):
            ...
            # prob_2d = np.asarray(prob).reshape(num_samples, vocab_size)
